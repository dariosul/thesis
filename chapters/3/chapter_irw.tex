...
Adopting this technique is useful in practical scenarios where synchronization between mobile units is not available \cite{GeoLoc}. Typical example of such setup includes the observed time difference of arrival (O-TDOA) technique used in WCDMA (3G) networks to estimate the location of mobile units. In WCDMA networks only the base stations are synchronized with each other, but the mobile unit is unsynchronized with base stations. 
