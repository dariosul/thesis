...
r_i = g_i \frac{S}{d_i^\alpha} + \varepsilon_i
\end{equation}
...
\right] ,
\end{equation}
and $\BZ$ are (estimated) normalized energy measurements for the case of the single radiating source. 


\subsection{Reformulation}

There are two things to note about (\ref{eq:3.35}). First, (\ref{eq:3.35}) is a \textit{nonlinear} least square objective function because the vector $\BH$ is a nonlinear function of the $n$  unknown source coordinates, where $n$ is the dimention of the location coordinates. Second, although there are $n + 1 \leq m$ unknowns, including the unknown acoustic energy $S$ radiated from the source. 
To eliminate the unknown $S$ \cite{ShengHu}  propose first to compute the ratio $k_{ij}$ of the calibrated energy readings from $i$th and $j$th sensor as
\begin{equation} \label{eq:3.36}
\nonumber
k_{ij} = \left[ \frac{z_i / g_i}{z_j / g_j} \right]^{-1/2}
\end{equation}
for $i = 1, 2, \ldots, m-1$, and $j = i+1, \ldots, m$. For the case $0 < k_{ij} \neq 1$ all possible source coordinates $\Bx$ reside on a $n$-dimensional hyper-sphere described by the equation:
...
\end{equation}
For the case when $k_{ij} = 1$ the possible source locations reside on the hyperplane between sensors $\Ba_i$ and $\Ba_j$:
\begin{equation}
\Bx^T \Biota_{ij} = \tau_{ij}
\end{equation}
where $\Biota_{ij} = \Ba_i - \Ba_j$ and $\tau_{ij} = (\|\Ba_i\|^2 - \|\Ba_j\|^2) / 2$. 

Let $I_1$ and $I_2$ be two index sets such that $\forall$  $\{i, j \in I_1; 1 \leq i \leq m-1, i+1 \leq j \leq m \}$ it is true that $0 < k_{ij} \neq 1$ and $\forall$  $\{i, j \in I_2; 1 \leq i \leq m-1, i+1 \leq j \leq m \}$  $k_{ij} = 1$.
Let $l_1$ and $l_2$ denote the number of elements in sets $I_1$ and $I_2$ respectively. Then the unknown location of the source can be found via minimization of the criterion that is equivalent to (\ref{eq:3.35}):
\begin{equation}%4
\Min \sum_{i,j \in I_1}^{} \left(\ \|\Bx - \Bc_{ij}\|^2 - \rho_{ij}^2\right)^2 + \sum_{i,j \in I_2}^{} \left( \Bx^T \Biota_{ij}  - \tau_{ij}\right)^2
\end{equation}
The first summation term can be written as


Methodology developed in section 3.1 can be applied here. 
