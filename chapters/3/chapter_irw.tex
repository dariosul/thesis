...
Adopting this technique is useful in practical scenarios where synchronization between mobile units is not available \cite{GeoLoc}. Typical example of such setup includes the observed time difference of arrival (O-TDOA) technique used in WCDMA (3G) networks to estimate the location of mobile terminals. In WCDMA networks only the base stations are synchronized with each other, but the mobile terminal is unsynchronized with base stations. 

Each TDOA measurement constrains the location of the mobile device to be on a hyperboloid with a constant range-difference between the two reference points. A TDOA measurement between base station $BS_i$ and $BS_0$ can be given by 
\begin{equation}
\nonumber
t_{i0} = (t_i - t_x) - (t_0 - t_x) = t_i - t_0
\end{equation}
where $t_x$ is the clock time of the mobile device, $t_i$ and $t_0$ are the time of arrival between the device and  stations $BS_i$ and $BS_0$ respectively. The equation can be written in terms of distance through scaling 
\begin{equation}
\nonumber
r_{i0} =(t_i - t_0)c = r_i - r_0 = \|\Ba_i - \Bx\| - \|\Ba_0 - \Bx\|
\end{equation}
where $c$ is the speed of signal propagation. Without loss of generality, the latter equation is valid with the assumption that the station $BS_0$ is placed at the origin of the coordinate system. 
...
\begin{equation}
d_i = r_i + \varepsilon_i, \mbox{ for } i = i = 1, 2, \ldots, m
\end{equation}
...
\Min_{\Bx \in R^n} F(\Bx)=\sum_{i=1}^m \left(d_i+\|\Bx\|-\|\Bx-\Ba_i\|\right)^2
 \end{equation}
 Unfortunately, finding the global solution of (\ref{eq:3.22}) turns out to be a very hard problem.
...
noise n in (1) is zero-mean and Gaussian distributed with covariance matrix C n , ML scheme reduces to the WNLS solution. The ML approach requires a high complexity when grid search is performed, and therefore global solution may not
...
\textit{
([14]  L.   Lu,   H.-C.   Wu,   K.   Yan,   and   S.   Iyengar,   “Robust   expectation maximization algorithm for multiple wideband acoustic source localization  in  the  presence  of  nonuniform  noise  variances,” IEEE Sensors J.,vol. 11, no. 3, pp. 536–544, Mar. 2011)}
