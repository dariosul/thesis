...
(from \cite{StLi} adapted but needs further adaptation)

The source localization problem from range or range-difference measurements discussed earlier is related to the problem of source localization from energy measurements (citation). The energy measurement based source localization is based on the fact that the evergy of the signal received by the $i$th sensor over a (relatively small) time interval is inversely proportional to $\| \Bx - \Ba_i \|$, for $i = 1, \ldots,m$. For notational convenience it is assumed that there are $m$ sensors.

Using this fact and somesimple manipulations (see, e.g., [5] for details), it is possible to obtain an
equation in the unknown vector x that
is somewhat similar to (2), namely: