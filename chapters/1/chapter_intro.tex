...
Different technologies have been used to build geolocation systems. Several global navigation satellite systems are available for both military and civilian use and include GPS(US), Galileo(EU), GLONASS(Russia), BeiDou(China). Atomic clocks onboard the satellites allow high-accuracy timing of how long it takes signals to reach them.
% Satellites are equipped with atomic clocks that provide a high-accuracy timing signal to  allow a receiver to calculate the time that it takes the signal to reach it. 
Time measurements can be used to calculate range since velocity of the signal is known. By using range measurements from multiple satellites, trilateration can be used to determine the position.
%This information is used to calculate the position of the receiver by trilaterating the difference in received signals from multiple satellites \textit{cite Wiki}.
