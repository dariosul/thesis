...
GPS is also widely used in navigation of other vehicles such as airplanes, ships, and heavy equipment. It has also been used for monitoring movements rather than navigating, for example in fitness trackers such as FitBit and Garmin \textit{FitBit and Garmin websites} which automatically collect data about the user's activities (distance run, speed, routes taken).
...
As a result there has been a desire for new location-based services with improved accuracy and coverage.
Novel solutions have been developed which attempt to integrate various wireless technologies with GPS. One such approach is called Assisted GPS (A-GPS), which distributes data and processing over a network of cellular towers equipped with GPS-enabled servers. This can greatly reduce the search space and time to first fix on location. \textit{TODO cite Richton}
