...
Let $\BX$ denote  an $m$ by $n$ distance matrix 
\begin{equation}
\nonumber
...
\right)
\end{equation}
The multidimentional similarity matrix is then defined by $\BQ = \BX\BX^T$ which can also be expressed in terms of pairwise distances between sensor nodes and error-free range measurements. Since the exact distances between the sourse $\Bx$ and sensors are not available, the noisy range measurements $r_i$ are used to construct an approximate version of $\BD$, denoted $\hat{\BD}$ as
\begin{equation}
\nonumber
...
r_1^2 + r_m^2 - \|\Ba_1 - \Ba_m\|^2 & r_2^2 + r_m^2 - \|\Ba_1 - \Ba_m\|^2 & \ldots & 2r_m^2\\
\end{array}
\right)
\end{equation}
...
\end{equation} 
where $\BOmega$ is an unknown rotation matrix to be determined. The estimate of the unknown rotation matrix $\BOmega$ and source location $\Bx$ can be obtained by solving an overdetermined system of linear equations \cite{classMDS}.

So and Chan \cite{genMDS} propose to perform the position estimation in the noise subspace. 

\textit{In the practical situations of nonzero range errors, will not be of rank 2 but has full rank.
It is assumed that range measurement errors between the sensors and source $\Bx$ are i.i.d. Gaussian variables with zero mean and \textit{known} variance. }
