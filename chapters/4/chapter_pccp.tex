...
%\end{equation}
The convex approximation of the objective in (\ref{eq:F}) can now be derived as follows
\begin{equation}
\nonumber
\begin{aligned}
...
\end{eqnarray} 
for $ 1 \leq i \leq m$. 
Placing such bounds means that as iterations procede the new iterates (coordinates of possible source locations)  are restricted to lie within a  physically meaningfull region defined by algorithmic parameters $\delta_i$.
 
The constraints in (\ref{eq:4.12}a) are convex and fit into the form of basic CCP in (\ref{eq:ii}) with $f_i(\Bx) = \|\Bx - \Ba_i\| - r_i -  \delta_i $ and $g_i(\Bx) = 0$, while those in (\ref{eq:4.12}b) are in the form of (\ref{eq:DC}) with $f_i(\Bx) =  r_i -  \delta_i $ and $g_i(\Bx) = \|\Bx - \Ba_i\|$. Following CCP (see (\ref{eq:i}b)),  $g_i(\Bx) = \|\Bx - \Ba_i\|$ is  linearized around iterate $\Bx_k$ to 
\begin{equation}
\nonumber
...
\end{equation}
which now fits into (\ref{eq:ii}b), or equivalently
\setcounter{abc}{0}
...
%\setlength{\belowdisplayskip}{0pt} \setlength{\belowdisplayshortskip}{0pt}
\begin{eqnarray} \label{eq:PCCP}% eq:4.16
\setcounter{abc}{1}
...
\item
Apply a “global” localization algorithm such as those in \cite{BeckStLi} to generate an approximate LS solution, then take it as the initial point to run the proposed algorithm. The algorithm can be now outlined as follows.
\end{enumerate}
